...
Haroon_Blob = 'DefaultEndpointsProtocol=https;AccountName=staticappaghaharoon;AccountKey=rwzgjJW9892D5VlTRZZW7L0ZIJHa1UjQ1l02FXFGArw4eqxH+U3Yedies6Kdb0/ZocOmFnMX1uGj+AStVfS40A==;EndpointSuffix=core.windows.net'
Haroon_Container = "uploads"
...
def upload():
    ...
    if file:
        ...
        blob_client = blob_service_client.get_blob_client(container=Haroon_Container, blob=filename)
        ...
        blob_url = f"https://{blob_service_client.account_name}.blob.core.windows.net/{Haroon_Container}/{filename}"
        media = Media(
            title=title,
            caption=caption,
            location=location,
...
db = SQLAlchemy(app)
blob_service_client = BlobServiceClient.from_connection_string(Haroon_Blob)
try:
    blob_service_client.create_container(Haroon_Container)
except Exception:
    pass
